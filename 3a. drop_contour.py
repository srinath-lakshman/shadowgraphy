...
sub_directory = r'needle_tip_45_mm_from_surface/impact_on_glass_r1/'
...
# plt.imshow(images[start_image-1], cmap='gray')
# plt.show()

ylimits = [y_wall-400, y_wall]
xlimits = [x_center-200, x_center+200]
...
for i in range(start_image, end_image+1):
    ...
    binary[50:80, 20:50] = 0
    binary[30:60, 330:360] = 0
    ...
    for y_avg in np.arange(np.min(binary_y), np.max(binary_y)+1,1):
        ...
        x_left_values = x_avg[np.argwhere(binary_x[indices] <= 200)[:,0]]
    ...
    for y_avg in np.arange(np.max(binary_y), np.min(binary_y)-1,-1):
        indices = np.argwhere(binary_y == y_avg)
        x_avg = binary_x[indices]
        x_right_values = x_avg[np.argwhere(binary_x[indices] >= 200)[:,0]]
